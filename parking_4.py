...
class SuperParkingBoy(object):
    ...
    def find_max_plot(self):
        for i in range(len(self.plot_list)):
            empty_rate=(self.plot_list[i].num_all-self.plot_list[i].num_cur)/self.plot_list[i].num_all
            if not self.max_plot.num_all:#最大空置率停车场空置率赋初值为-1
                max_empty_rate=-1
            else:
                max_empty_rate=(self.max_plot.num_all-self.max_plot.num_cur)/self.max_plot.num_all
